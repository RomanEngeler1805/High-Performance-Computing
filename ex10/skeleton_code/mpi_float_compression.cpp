        ...
        const size_t bufsize, const size_t Nx, const size_t Ny, const double tol)
{
    ...
    // 2.)
    int f = sizeof(FileHeader);
    int bp = sizeof(BlockHeader);

    // 3.)
    int ck = sizeof(unsigned char)* cbytes; // lenght of data chunk
    int offset_data;
    MPI_Exscan(&ck, &offset_data, 1, MPI_OFFSET, MPI_SUM, MPI_COMM_WORLD); // communicate
    int start = f+ size* bp+ offset_data; // add length_fileheader+ length_blockheader+ data_length

    // 4.)
    BlockHeader bheader;
    bheader.start = start; //  offset from beginning of file
    ...
    MPI_File_open(MPI_COMM_WORLD, fname.data(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &fh);

    // 2.)
    if(isroot) MPI_File_write(fh, &fheader, f, MPI_CHAR, &st); // char -> 1 byte

    int offset_header = f+ rank* bp;
    MPI_File_write_at_all(fh, offset_header, &bheader, bp, MPI_CHAR, &st); // count = bp

    // 3.)
    MPI_File_write_at_all(fh, start, &cbuf[0], ck, MPI_CHAR, &st);
        ...
        size_t& cbytes, size_t& bufsize, size_t& Nx, size_t& Ny, double& tol)
{
    ...
    MPI_File_open(MPI_COMM_WORLD, fname.data(), MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);
    ...
    int bp = sizeof(BlockHeader);
    int offset_fh = 0;
    int offset_bh = f+ rank* bp;

    MPI_File_read_at_all(fh, offset_fh, &fheader, f, MPI_CHAR, &st); // no offset
    MPI_File_read_at_all(fh, offset_bh, &bheader, bp, MPI_CHAR, &st); // no offset
    ...
    unsigned char* cbuf; 
    cbuf = new unsigned char [bheader.bufsize];

    MPI_File_read_at_all(fh, bheader.start, &cbuf[0], bheader.compressed_bytes, MPI_CHAR, &st);
