...
static inline float sse_red_4(const float* const ary, const size_t N)
{
    ...
    for (size_t i= 0; i< N; i+= simd_width){
        ...
        const __m128 x4 = _mm_load_ps(ary + i);
        // add vector to sum
        sum = _mm_add_ps(sum, x4);
    }

    float sum_return = sum[0]+ sum[1]+ sum[2]+ sum[3];

    // https://software.intel.com/en-us/forums/intel-isa-extensions/topic/346695
    return sum_return; // the function returns the summation of all elemnts in ary
}
...
static inline double sse_red_2(const double* const ary, const size_t N)
{
    ...
    // change intel intrinsics to doubel !!!!!!!!!!!!!!!!!!!!!!
    ...
    for (unsigned i= 0; i< N; i+= simd_width){
        // load data in register
        const __m128d x2 = _mm_load_pd(ary + i);
        // add vector to sum
        sum = _mm_add_pd(sum, x2);
    }

    double sum_return = sum[0]+ sum[1];
...
void benchmark_omp(const size_t N, T(*func)(const T* const, const size_t), const size_t nthreads, const string test_name)
{
    ...
    // above.
    ///////////////////////////////////////////////////////////////////////////
    initialize(ary, N* nthreads);

    // T res_gold = gold_red(ary, nthreads*N);
    T res_gold = 0.0;
    gold_red(ary, nthreads*N); // warm-up
    auto t1 = Clock::now();
    // collect 10 samples.  Note: we are primarily interested in the
    // performance here, not the result
    for (int i = 0; i < 12; ++i)
    ...
    // test
    T gres = 0.0;
    (*func)(ary, nthreads*N); // warm-up
    auto tt1 = Clock::now();
    // again 10 samples
    #pragma omp parallel for
    for (int i = 0; i < 12; ++i)
        gres += (*func)(ary, nthreads*N);
    auto tt2 = Clock::now();
    double gt = chrono::duration_cast<chrono::nanoseconds>(tt2 - tt1).count();
