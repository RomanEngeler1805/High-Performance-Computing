...
int main (int argc, char** argv)
{
  ...
  net.addConv2D< 28, 28,  1,   4,   4,   16,   2,2,    0,0>();
  // output of first conv has sizes (28 -8 +2*0)/2+1 = 11
  net.addLReLu< 13 * 13 * 16 >();

  net.addConv2D< 13, 13,  16,   4,   4,   16,   1,1,    0,0>();
  // output of first conv has sizes (11 -6 +2*0)/1+1 = 6
  net.addLReLu< 10 * 10 * 16 >();

  net.addConv2D<  10,  10,  16,   4,   4,  16,   1,1,    0,0>();
  // output of first conv has sizes ( 6 -4 +2*0)/1+1 = 3
  net.addLReLu< 7 * 7 * 16 >();

  net.addConv2D<  7,  7,  16,   4,   4,  16,   1,1,    0,0>();
  // output of first conv has sizes ( 6 -4 +2*0)/1+1 = 3
  net.addLReLu< 4 * 4 * 16 >();

  net.addConv2D<  4,  4, 16,   4,   4,  10,   1,1,    0,0>();
