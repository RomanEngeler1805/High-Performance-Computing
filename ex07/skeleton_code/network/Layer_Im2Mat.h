...
#pragma omp parallel for
    for (int bc=0; bc<BS; bc++)
      ...
      for (int ox = 0; ox < OpX; ox++)
        ...
        for (int fx = 0; fx < KnX; fx++)
          for (int ic = 0; ic < InC; ic++){
            int inx = Sx* (ox- 1)+ fx- 2* Px;
            int iny = Sy* (oy- 1)+ fy- 2* Py;
            if((0 < inx && inx < InX) && (0 < iny && iny < InY))
              OUT[bc][oy][ox][fy][fx][ic] = INP[bc][iny][inx][ic];
          }
  }
      ...
      for (int ox = 0; ox < OpX; ox++)
        ...
        for (int fx = 0; fx < KnX; fx++)
          for (int ic = 0; ic < InC; ic++){
            ...
            int iny = Sy* (oy- 1)+ fy- 2* Py;
            if((0 < inx && inx < InX) && (0 < iny && iny < InY))
              dLdINP[bc][iny][inx][ic] = dLdOUT[bc][oy][ox][fy][fx][ic];
          }
  }
