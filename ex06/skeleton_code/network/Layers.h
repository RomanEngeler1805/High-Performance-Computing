...
struct LinearLayer: public Layer
{
               ...
               const std::vector<Params*>& param) const override
  {
    ...
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans,
                batchSize, nOutputs, nInputs, 1.0, inputs, nInputs, weight, nOutputs, 1.0, output, nOutputs);
  }
               ...
               const std::vector<Params*>& grad)  const override
  {
    ...
    // TODO: Implement BackProp to compute weight gradient: dError / dWeights
    {
      ...
      cblas_dgemm(CblasRowMajor, CblasTrans, CblasNoTrans,
                nInputs, nOutputs, batchSize, 1.0, inputs, nInputs, deltas, nOutputs, 0.0, grad_W, nOutputs);
      // batchSize, nOutputs, nInputs, 1.0, inputs, nInputs, deltas, nOutputs, 0.0, grad_W, nOutputs);
    }
    ...
    // TODO: Implement BackProp to compute dEdO of prev layer
    {
      ...
      cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasTrans,
                batchSize, nInputs, nOutputs, 1.0, deltas, nOutputs, weight, nOutputs, 0.0, errinp, nInputs);
    }
  }
...
struct TanhLayer: public Layer
{
               ...
               const std::vector<Params*>& grad)  const override
  {
    ...
    for(int i= 0; i< batchSize* size; i++){
      errinp[i] = deltas[i]* (1- pow(tanh(inputs[i]),2));
    }
